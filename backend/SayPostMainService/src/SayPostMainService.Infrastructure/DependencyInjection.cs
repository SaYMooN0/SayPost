...
using SayPostMainService.Infrastructure.persistence.repositories;
...
public static class DependencyInjection
{
    ...
    private static IServiceCollection AddPersistence(this IServiceCollection services, IConfiguration configuration) {
        string dbConnectionString = configuration.GetConnectionString("MainServiceDb")
                                    ?? throw new Exception("Database connection string is not provided.");
        services.AddDbContext<MainDbContext>(options => options.UseNpgsql(dbConnectionString));

        services.AddScoped<IAppUsersRepository, AppUsersRepository>();
        services.AddScoped<IDraftPostsRepository, DraftPostsRepository>();
        services.AddScoped<IPublishedPostsRepository, PublishedPostsRepository>();
