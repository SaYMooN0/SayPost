...
public record class ListUserFollowingIdsForStatisticsQuery(AppUserId UserId)
    : IRequest<ErrOr<IReadOnlyCollection<AppUserId>>>;

internal class ListUserFollowingIdsForStatisticsQueryHandler :
    IRequestHandler<ListUserFollowingIdsForStatisticsQuery, ErrOr<IReadOnlyCollection<AppUserId>>>
{
    ...
    public ListUserFollowingIdsForStatisticsQueryHandler(
    ...
    public async Task<ErrOr<IReadOnlyCollection<AppUserId>>> Handle(
        ListUserFollowingIdsForStatisticsQuery query, CancellationToken cancellationToken
    ) {
        AppUser? user = await _appUsersRepository.GetByIdAsNoTracking(query.UserId);
        if (user is null) {
            return ErrFactory.NotFound(
                "User not found", $"Could not get user followings. User {query.UserId} was not found"
            );
        }
        ...
        if (!isAuthenticated && user.StatisticsVisibilitySettings.FollowingOnlyForFollowers) {
        ...
        if (
            ...
            || !user.StatisticsVisibilitySettings.FollowingOnlyForFollowers
            || userWithFollowings.IsFollowedBy(viewerId)
        ) {
