...
public record class ListUserLikedPostsForStatisticsQuery(AppUserId UserId)
    : IRequest<ErrOr<IReadOnlyCollection<PublishedPost>>>;

internal class ListUserLikedPostsQueryHandler :
    IRequestHandler<ListUserLikedPostsForStatisticsQuery, ErrOr<IReadOnlyCollection<PublishedPost>>>
{
    ...
    public async Task<ErrOr<IReadOnlyCollection<PublishedPost>>> Handle(
        ListUserLikedPostsForStatisticsQuery query, CancellationToken cancellationToken
    ) {
        AppUser? user = await _appUsersRepository.GetByIdAsNoTracking(query.UserId);
        if (user is null) {
            return ErrFactory.NotFound(
                "User not found", $"Could not get user's liked posts. User {query.UserId} was not found"
            );
        }

        if (!user.StatisticsVisibilitySettings.LikedPostsOnlyForFollowers) {
            return await ExtractLikedPosts(user);
        }
    ...
    private async Task<ErrOr<IReadOnlyCollection<PublishedPost>>> ExtractLikedPosts(AppUser user) {
