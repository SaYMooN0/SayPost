...
using SayPostFollowingsService.Infrastructure.persistence.repositories;
...
public static class DependencyInjection
{
    ...
    private static IServiceCollection AddPersistence(this IServiceCollection services, IConfiguration configuration) {
        string dbConnectionString = configuration.GetConnectionString("FollowingServiceDb")
                                    ?? throw new Exception("Database connection string is not provided.");
        services.AddDbContext<FollowingDbContext>(options => options.UseNpgsql(dbConnectionString));
